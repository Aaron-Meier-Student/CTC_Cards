...
class Solver {
    ...
    css(filters) {
        ...
        return `filter: invert(${fmt(0)}%) sepia(${fmt(1)}%) saturate(${fmt(2)}%) hue-rotate(${fmt(3, 3.6)}deg) brightness(${fmt(4)}%) contrast(${fmt(5)}%) opacity(50%);`;
    }
}
...
function parseRgbString(rgbString) {
    const rgbArray = rgbString.split('", "').map((color) => {
